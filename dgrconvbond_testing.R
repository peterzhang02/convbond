...
stock_prices <- seq(0.001, 0.0240, length.out = stock_runs)
payoffs <- vector("numeric", stock_runs)

for (j in 1:stock_runs) {
  stock_0 <- stock_prices[j]
...
print(binomial_tree_payoff[1, 1])

payoffs[j] <- binomial_tree_payoff[1, 1]
}
...
# plot of conv bond value vs stock price
